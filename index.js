...
app.use((req, res, next) => {
  ...
  console.log(
    `Request URL: ${req.url}, Method: ${req.method}, Language: ${
      req.cookies.lang || "en"
    }`
  );
  next();
...
app.get("/register", (req, res) => {
  ...
  if (usertoken) {
    return res
      .status(400)
      .send(
        "you cant access this page please logout first to register the data "
      );
  }
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "User registered successfully", registrationId });
  } catch (error) {
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (user) {
      ...
      if (isMatch) {
        const usertoken = jwt.sign({ phone: phone }, secretKey, {
          expiresIn: "1h",
        });
...
app.get("/applied-jobs", authenticateusertoken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("An error occurred while serving the applied jobs page.");
  }
});
...
app.post("/api/track_reference", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while tracking the reference number" });
  }
});
...
app.get("/notifications/:employee_id", async (req, res) => {
  ...
  try {
    ...
    const userMessages = await SeparateNotification.find({
      recipients: { $in: [employee_id] },
    }).sort({ timestamp: -1 });
